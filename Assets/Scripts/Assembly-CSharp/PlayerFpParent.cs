...
public class PlayerFpParent : MonoBehaviour
{
	...
	private const float WALK_OFFSET_MAGNITUDE = 0.02f;
	...
	private void Awake()
	{
		...
		positionSpring = new Spring(150f, 10f, -Vector3.one * 0.2f, Vector3.one * 0.2f);
		rotationSpring = new Spring(70f, 6f, -Vector3.one * 15f, Vector3.one * 15f);
	...
	private void Update()
	{
		...
		if (!actor.IsSeated())
		{
			target = Mathf.Clamp01(actor.Velocity().magnitude / 5f) * ((!aiming) ? 0.02f : 0.003f);
		}
	...
	private void KickCamera(Vector3 kick)
	{
		fpKickAction.Start();
		kickEuler = kick * 0.7f;
	}
