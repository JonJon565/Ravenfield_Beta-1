...
public class AiActorController : ActorController
{
	...
	private const float LEAD_SWAY_MAGNITUDE = 0.1f;

	private const float LEAD_NOISE_MAGNITUDE = 0.05f;

	private const float SWAY_MAGNITUDE = 1f;

	private const float VEHICLE_STUCK_DISTANCE = 0.4f;

	private const float VEHICLE_STUCK_TIME = 2f;

	private const float VEHICLE_STUCK_RECOVER_TIME = 1f;

	private const float MAX_RECENT_ANTI_STUCK_EVENTS = 3f;
	...
	private const float COS_45 = 0.7071f;
	...
	private const float AI_FIRE_RECTANGLE_BOUND = 1f;
	...
	private const float AIM_BASE_SWAY = 0.002f;

	private const float AIM_MAX_SWAY = 0.05f;
		...
		while (true)
		{
			if (actor.IsSeated() && actor.seat.vehicle != null && actor.IsDriver())
			{
				...
				if (hasPath)
				{
					...
					else if (Time.time > lastSampleTime + 2f)
					{
	...
	{
		if ((float)recentAntiStuckEvents > 3f)
		{
	...
	private IEnumerator AiWeapon()
	{
		...
		while (true)
		{
			...
			bool insideAimCube = Vector3.Dot(deltaTarget, forward) > 0f && Mathf.Abs(a) < 1f * allowedAimSpread && Mathf.Abs(b) < 1f * allowedAimSpread;
		...
		while (true)
		{
			...
			if (!skipNextScan && !HasTarget())
			{
				...
				else
				{
					...
					if (hasPath)
					{
						facingDirection += Velocity().normalized * 0.5f;
					}
					facingDirection += SquadFacingBias();
	...
	private Vector3 WeaponLead()
	{
		...
		float num3 = num2 * (1f + 0.1f * (vector4.x + vector4.z) + UnityEngine.Random.Range(-0.05f, 0.05f));
		Vector3 vector10 = vector9 * num3 - Physics.gravity * Mathf.Pow(num3, 2f) / 2f;
		return vector10 + vector5 * 1f;
	}
	...
	public override Vector3 FacingDirection()
	{
		...
		return facingDirection * Vector3.forward + vector * (0.002f + 0.05f * fatigue);
	}
	...
	public bool FindCover()
	{
		...
		inCover = false;
		cover = CoverManager.instance.ClosestVacant(base.transform.position);
		if (HasCover())
		{
			CancelPath();
			cover.taken = true;
