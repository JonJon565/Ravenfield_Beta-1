...
public class ActorManager : MonoBehaviour
{
	...
	private void FillEmptySlotsWithAI()
	{
		...
		{
			CreateAIActor(i % 2, (float)i / (float)maxActors);
		}
	}
	...
	public static void Explode(Vector3 point, ExplodingProjectile.ExplosionConfiguration configuration)
	{
		...
		foreach (Actor item in list)
		{
			...
			if (!item.dead)
			{
				item.Damage(configuration.damage * num, configuration.balanceDamage * num2, item.CenterPosition(), vector.normalized * configuration.force * num2);
			}
			else
			{
